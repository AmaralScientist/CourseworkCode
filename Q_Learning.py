##
 #  This program implements the Q-learning algorithm for the racetrack problem where an  
...
# This method implements the Q-learning algorithm to train the agent.    
def train_agent( training_dictionary, wall_behind_finish_coordinates, x_chunk_of_track, y_chunk_of_track, starting_line_coordinates, finish_line_coordinates, parsed_track, crash_option, number_training_episodes, final_training ):
    ...
    for episode_index in range( 0, int(number_training_episodes) ):
        ...
        for iteration_index in range( 0, 10000 ):
            track_location = check_racetrack_location( this_position, parsed_track )
            if track_location == "#":
                print("Car crashed: ", track_location)
                break
            ...
            if (this_position[0] in x_chunk_indices) and (this_position[1] in y_chunk_indices):
                ...
                action_to_take, this_q_value = determine_action( this_position, this_velocity, 0.2 )
            ...
            #print("\nUpdated position before evaluation: ", updated_position)
                ...
                while track_index < len(track_positions):
                    if track_positions[track_index] == ".":
                        ...
                        #print("A crash occurred")
                         ...
                         updated_velocity = (0,0)
                         break
            #print("Updated position after evaluation: ", updated_position)    
            if updated_position != (-1, -1):
                # Now update the q values
                # a' will be selected via a purely greedy strategy
                for positions in training_dictionary:
                ...
                this_q_value_for_a_prime = max( list_of_q_values )
                largest_q_index_for_a_prime = list_of_q_values.index( max(list_of_q_values) )
                purely_greedy_a_prime = list_of_actions[ largest_q_index_for_a_prime ]

                training_dictionary[positions][velocities][action_to_take] = (( 1 - learning_rate ) * this_q_value) + ( learning_rate * (reward + (discount * training_dictionary[updated_position][updated_velocity][purely_greedy_a_prime])) )
    ...
    print("attempts_for_finish: ", attempts_for_finish)
...
with open ("/Users/michelleamaral/Desktop/Intro_to_Machine_Learning/Programming_Project_6/Outputs/" + output_file_name_final + "_track_output_Q_Learning_" + number_training_episodes + "_training_episodes", 'w') as outputFile:

    outputFile.write( "\nTrack: " + output_file_name_final + "\n" )
    outputFile.write( "Algorithm: Q-Learning" + '\n')
