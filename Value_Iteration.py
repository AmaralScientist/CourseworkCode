##
 #  This program implements the Value Iteration algorithm for the racetrack problem where  
 #  an agent (a race car) is trained and tested on various racetracks via reinforcement 
...
# This method generates a dictionary for use in training that is indexed by
# position : velocity. All values are initialized to random values from
# a normal distribution with mean = 0 and stdev = 1. V(s) for the
# finish line coordinates are initialized to 0.
def create_values_dictionary( x_dimension, y_dimension, finish_line_coordinates ):
    # Generate all possible positions of racetrack
    possible_positions = generate_possible_positions( x_dimension, y_dimension )
    ...
    # Create values dictionary and initialize these values
    position_dictionary_list = []    
    for positions_index in range( 0, len(possible_positions) ):
        velocity_dictionary_list = []
        for velocities_index in range( 0, len(velocities) ):
            q_value = np.random.normal(0, 1, 1)[0]
            velocity_dictionary_list.append( q_value )
        velocity_dictionary = dict( zip(velocities, velocity_dictionary_list) )
        position_dictionary_list.append( velocity_dictionary )
    values_dictionary = dict( zip(possible_positions, position_dictionary_list) )

    # Initialize the positions corresponding to the finish line to zero
    for items in range( 0, len(finish_line_coordinates) ):
        if finish_line_coordinates[items] in values_dictionary:
            for velocities in values_dictionary[ finish_line_coordinates[items] ]:
                values_dictionary[ finish_line_coordinates[items] ][velocities] = 0
...
# This method resets the q values for the finish line coordinates to zero
def zero_finish_line_coordinates_in_values_dict( training_dictionary, finish_line_coordinates ):
    # Initialize finish line coordinate Q values to zero
    for items in range( 0, len(finish_line_coordinates) ):
        if finish_line_coordinates[items] in training_dictionary:
            for velocities in training_dictionary[ finish_line_coordinates[items] ]:
                training_dictionary[ finish_line_coordinates[items] ][velocities] = 0
...
# This method calculates the velocity component of s'.
def calculate_new_velocity( current_velocity, acceleration ):
    minimum_velocity = -5
    maximum_velocity = 5
    ...
    velocity = ( updated_x_velocity, updated_y_velocity )        
    return( velocity )
 
 
# This method calculates the updated position component of s'.
def calculate_new_position( current_position, updated_velocity ):
    ...
    position = ( updated_x_position, updated_y_position )    
    return( position )
...
# This method extracts the policy just prior to the start of the time trials
def obtain_policy( q_value_dictionary, policy_dictionary ):
    for positions in q_value_dictionary:
        for velocities in q_value_dictionary[positions]:
            list_of_actions = []
            list_of_q_values = []
            for actions in q_value_dictionary[positions][velocities]:
                list_of_actions.append( actions )
                list_of_q_values.append( q_value_dictionary[positions][velocities][actions] )
            ...
            policy_dictionary[positions][velocities] = action_to_take
    return( policy_dictionary )
                

# This method implements the value iteration algorithm.    
def value_iteration( q_value_dictionary, values_dictionary, policy_dictionary, wall_behind_finish_coordinates, finish_line_coordinates, parsed_track, crash_option, number_training_episodes ):
    # Set the Bellman Error Magnitude
    error_threshold = 0.001
    gamma = 0.9
    
    # Re-zero the finish line q values
    q_value_dictionary = zero_finish_line_coordinates( q_value_dictionary, finish_line_coordinates )
    # Re-zero the finish line V(s) values
    values_dictionary = zero_finish_line_coordinates_in_values_dict( values_dictionary, finish_line_coordinates )
    
    # Generate possible actions that can be taken by the agent
    possible_acceleration_values = [-1, 0, 1]
    actions = generate_combinations( possible_acceleration_values )
    
    for training_index in range( 0, int(number_training_episodes) ):
        prior_iteration_values_dictionary = copy.deepcopy( values_dictionary )
        
        # Iterate over all possible states
        for positions in values_dictionary:
            for velocities in values_dictionary[positions]:
                track_location = check_racetrack_location( positions, parsed_track )
                if track_location == "#":
                    values_dictionary[positions][velocities] = -50
                    continue
                ...
                for action in actions:
                    ...
                    # Act on the environment (80% of the time)
                    updated_velocity = calculate_new_velocity( velocities, action )
                    updated_position = calculate_new_position( positions, updated_velocity )
                    
                    if ( updated_position[0] < 0) or (updated_position[0] >= int(x_dimension)):
                        value_of_updated_state = -50
                    elif ( updated_position[1] < 0 or updated_position[1] >= int(y_dimension) ):
                        value_of_updated_state = -50
                    else:
                        # Check for crash
                        if positions != updated_position:
                            intervening_coordinates = bresenham_algorithm( positions, updated_position, parsed_track )
                            # Evaluate that path    
                            track_positions = []        
                            ...
                            while track_index < len(track_positions):
                                if track_positions[track_index] == ".":
                                    ...
                                    value_of_updated_state = prior_iteration_values_dictionary[updated_position][updated_velocity]
                    ...
                    # Act on the environment (20% of the time car cannot accelerate)
                    updated_velocity_if_no_acceleration = calculate_new_velocity( velocities, (0,0) )
                    updated_position_if_no_acceleration = calculate_new_position( positions, updated_velocity_if_no_acceleration )
                    
                    if (updated_position_if_no_acceleration[0] < 0) or (updated_position_if_no_acceleration[0] >= int(x_dimension)):
                        value_of_updated_state_if_no_acceleration = -50
                    elif ( updated_position_if_no_acceleration[1] < 0 or updated_position_if_no_acceleration[1] >= int(y_dimension) ):
                        value_of_updated_state_if_no_acceleration = -50
                    # Check for crash
                    if positions != updated_position:
                        intervening_coordinates = bresenham_algorithm( positions, updated_position, parsed_track )
                        ...
                        while track_index < len(track_positions):
                            if track_positions[track_index] == ".":
                                updated_position = intervening_coordinates[track_index]
                                track_index = track_index + 1
                                value_of_updated_state_if_no_acceleration = prior_iteration_values_dictionary[updated_position][updated_velocity]
                            elif track_positions[track_index] == "F":
                                #print("Crossed the finish!\n\n\n\n\n\n")
                                updated_position = (-1, -1)
                                break
                    ...
                    expected_value = ( 0.8 * value_of_updated_state ) + ( 0.2 * value_of_updated_state_if_no_acceleration )
                    q_value_dictionary[positions][velocities][action] = reward + ( gamma * expected_value )
                    list_of_q_values.append( reward + ( gamma * expected_value ) )
                    
                this_q_value = max( list_of_q_values )
                largest_q_index = list_of_q_values.index( max(list_of_q_values) )
                values_dictionary[positions][velocities] = this_q_value
        ...
        for positions in values_dictionary:
            for velocities in values_dictionary[positions]:
                difference_between_values = abs( prior_iteration_values_dictionary[positions][velocities] - values_dictionary[positions][velocities] )
                value_differences.append( difference_between_values )
                        
        maximum_difference = max( value_differences )
        print("maximum difference: ", maximum_difference)
                
        if maximum_difference < error_threshold:
            policy = obtain_policy( q_value_dictionary, policy_dictionary )
            return( policy )
...
def time_trial( policy, starting_line_coordinates, parsed_track, finish_line_coordinates, wall_behind_finish_coordinates, crash_option ):
    ...
    number_of_moves = 0
    for time_trial_index in range( 0, 20000 ):
    
        # Locate starting state and q values in the training dictionary
        for positions in policy:
            if positions == this_position:
                for velocities in policy[positions]:
                    if velocities == this_velocity:
                        action_to_take = policy[positions][velocities]
                    
        updated_velocity = calculate_new_velocity( this_velocity, action_to_take )
        #print("Updated velocity: ", updated_velocity)

        updated_position = calculate_new_position( this_position, updated_velocity )
    ...
    print("Number of moves it took: ", number_of_moves )
...
# Identify the coordinates of the finish line
starting_line_coordinates, finish_line_coordinates = get_start_and_finish_line_coordinates( parsed_track )
# Create dictionary to train model by tracking q values
q_value_dictionary = create_training_dictionary( x_dimension, y_dimension, finish_line_coordinates )
# Create dictionary to train model by tracking V(s)
values_dictionary = create_values_dictionary( x_dimension, y_dimension, finish_line_coordinates )
# Create a policy dictionary to store the best action to take at 
policy_dictionary = create_values_dictionary( x_dimension, y_dimension, finish_line_coordinates )
...
for starting_index in range( 0, len(backup_coordinate_list) ):
    ...
    policy = value_iteration( q_value_dictionary, values_dictionary, policy_dictionary, wall_behind_finish_coordinates, finish_line_coordinates, parsed_track, crash_option, number_training_episodes )


with open ("/Users/michelleamaral/Desktop/Intro_to_Machine_Learning/Programming_Project_6/Outputs/" + output_file_name_final + "_track_output_Value_Iteration", 'w') as outputFile:

    outputFile.write( "\nTrack: " + output_file_name_final + "\n" )
    outputFile.write( "Algorithm: Value Iteration" + '\n')
    ...
    for races_index in range(0, 100):
        number_of_moves = time_trial( policy, starting_line_coordinates, parsed_track, finish_line_coordinates, wall_behind_finish_coordinates, crash_option )
